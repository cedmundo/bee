...
jit_value_t bee_compile_node(jit_function_t f, struct bee_ast_node *node, struct bee_compiler_error *error) {
    ...
    switch (node->type) {
        ...
        case BEE_AST_NODE_BIN_SUB:
            return jit_insn_sub(f, bee_compile_node(f, node->left, error),
                                bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_MUL:
            return jit_insn_mul(f, bee_compile_node(f, node->left, error),
                                bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_DIV:
            return jit_insn_div(f, bee_compile_node(f, node->left, error),
                                bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_REM:
            return jit_insn_rem(f, bee_compile_node(f, node->left, error),
                                bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_EQ:
            return jit_insn_eq(f, bee_compile_node(f, node->left, error),
                               bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_NE:
            return jit_insn_ne(f, bee_compile_node(f, node->left, error),
                               bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_GT:
            return jit_insn_gt(f, bee_compile_node(f, node->left, error),
                               bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_GE:
            return jit_insn_ge(f, bee_compile_node(f, node->left, error),
                               bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_LT:
            return jit_insn_lt(f, bee_compile_node(f, node->left, error),
                               bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_LE:
            return jit_insn_le(f, bee_compile_node(f, node->left, error),
                               bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_LSH:
            return jit_insn_shl(f, bee_compile_node(f, node->left, error),
                                bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_RSH:
            return jit_insn_shr(f, bee_compile_node(f, node->left, error),
                                bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_BIT_AND:
            return jit_insn_and(f, bee_compile_node(f, node->left, error),
                                bee_compile_node(f, node->right, error));
        case BEE_AST_NODE_BIN_BIT_OR:
            return jit_insn_or(f, bee_compile_node(f, node->left, error),
                               bee_compile_node(f, node->right, error));
