...
static bool match_keyword_and_consume(struct bee_token *rest,
                                      enum bee_keyword_type keyword_type) {
...
struct bee_ast_node *bee_ast_node_new_binary(struct bee_token at_token,
                                             enum bee_ast_node_type type,
                                             struct bee_ast_node *left,
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_expr(struct bee_token *rest,
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_let_in(struct bee_token *rest,
                                      struct bee_error *error) {
    if (match_keyword(*rest, BEE_KEYWORD_LET)) {
        ...
        if (!match_punct(*rest, BEE_PUNCT_WALRUS)) {
            ...
            bee_set_error(error, BEE_ERROR_PARSER_WAS_EXPECTING_ASSIGN, rest->name,
                          rest->row, rest->col,
                          "Unexpected token `%.*s`, was expecting: `:=`", rest->len,
                          rest->data + rest->off);
            return NULL;
        }
        ...
        if (!match_keyword_and_consume(rest, BEE_KEYWORD_IN)) {
            ...
            bee_set_error(error, BEE_ERROR_PARSER_WAS_EXPECTING_IN, rest->name,
                          rest->row, rest->col,
                          "Unexpected token `%.*s`, was expecting: `in`");
            return NULL;
        }
        ...
        struct bee_ast_node *assign_node = bee_ast_node_new_binary(
                walrus_starting_at, BEE_AST_NODE_BIN_DUCK_ASSIGN, id_node,
                value_expr_node);
        struct bee_ast_node *let_in_node = bee_ast_node_new_binary(
                let_starting_at, BEE_AST_NODE_LET_IN_EXPR, assign_node, into_expr_node);
        return let_in_node;
    }
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_log_or(struct bee_token *rest,
                                      struct bee_error *error) {
    ...
    for (;;) {
        if (match_keyword(*rest, BEE_KEYWORD_OR)) {
            ...
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_LOG_OR, node,
                                           bee_parse_log_and(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
            continue;
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_log_and(struct bee_token *rest,
                                       struct bee_error *error) {
    ...
    for (;;) {
        if (match_keyword(*rest, BEE_KEYWORD_AND)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_LOG_AND, node,
                                           bee_parse_log_not(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
            continue;
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_log_not(struct bee_token *rest,
                                       struct bee_error *error) {
    if (match_keyword(*rest, BEE_KEYWORD_NOT)) {
        struct bee_token token = consume(rest);
        return bee_ast_node_new_unary(token, BEE_AST_NODE_UNA_LOG_NEG,
                                      bee_parse_log_not(rest, error));
    }
...
//                  | '<' bit_or | '<=' bit_or)*
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_rel(struct bee_token *rest,
                                   struct bee_error *error) {
    ...
    for (;;) {
        if (match_punct(*rest, BEE_PUNCT_D_EQ)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_EQ, node,
                                           bee_parse_bit_or(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
            continue;
        ...
        if (match_punct(*rest, BEE_PUNCT_NE)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_NE, node,
                                           bee_parse_bit_or(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
        ...
        if (match_punct(*rest, BEE_PUNCT_GT)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_GT, node,
                                           bee_parse_bit_or(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
        ...
        if (match_punct(*rest, BEE_PUNCT_GE)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_GE, node,
                                           bee_parse_bit_or(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
        ...
        if (match_punct(*rest, BEE_PUNCT_LT)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_LT, node,
                                           bee_parse_bit_or(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
        ...
        if (match_punct(*rest, BEE_PUNCT_LE)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_LE, node,
                                           bee_parse_bit_or(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_bit_or(struct bee_token *rest,
                                      struct bee_error *error) {
    ...
    for (;;) {
        if (match_punct(*rest, BEE_PUNCT_VBAR)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_BIT_OR, node,
                                           bee_parse_bit_xor(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
            continue;
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_bit_xor(struct bee_token *rest,
                                       struct bee_error *error) {
    ...
    for (;;) {
        if (match_punct(*rest, BEE_PUNCT_CARET)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_BIT_XOR, node,
                                           bee_parse_bit_and(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
            continue;
...
// bit_and = shift ('&' shift)*
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_bit_and(struct bee_token *rest,
                                       struct bee_error *error) {
    ...
    for (;;) {
        if (match_punct(*rest, BEE_PUNCT_AMPERSAND)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_BIT_AND, node,
                                           bee_parse_shift(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
            continue;
...
// shift = add ('>>' add | '<<' add)*
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_shift(struct bee_token *rest,
                                     struct bee_error *error) {
    ...
    for (;;) {
        if (match_punct(*rest, BEE_PUNCT_D_LT)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_LSH, node,
                                           bee_parse_add(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
            continue;
        ...
        if (match_punct(*rest, BEE_PUNCT_D_GT)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_RSH, node,
                                           bee_parse_add(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
...
// add = mul ('+' mul | '-' mul)*
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_add(struct bee_token *rest,
                                   struct bee_error *error) {
    ...
    for (;;) {
        if (match_punct(*rest, BEE_PUNCT_PLUS)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_ADD, node,
                                           bee_parse_mul(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
            continue;
        ...
        if (match_punct(*rest, BEE_PUNCT_MINUS)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_SUB, node,
                                           bee_parse_mul(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_mul(struct bee_token *rest,
                                   struct bee_error *error) {
    ...
    for (;;) {
        if (match_punct(*rest, BEE_PUNCT_ASTERISK)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_MUL, node,
                                           bee_parse_unary(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
            continue;
        ...
        if (match_punct(*rest, BEE_PUNCT_SLASH)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_DIV, node,
                                           bee_parse_unary(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
        ...
        if (match_punct(*rest, BEE_PUNCT_PERCENT)) {
            struct bee_token token = consume(rest);
            node = bee_ast_node_new_binary(token, BEE_AST_NODE_BIN_REM, node,
                                           bee_parse_unary(rest, error));
            if (bee_has_error(error)) {
                return node;
            }
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_unary(struct bee_token *rest,
                                     struct bee_error *error) {
    if (match_punct(*rest, BEE_PUNCT_PLUS)) {
        struct bee_token token = consume(rest);
        return bee_ast_node_new_unary(token, BEE_AST_NODE_UNA_ARI_POS,
                                      bee_parse_unary(rest, error));
    }

    if (match_punct(*rest, BEE_PUNCT_MINUS)) {
        struct bee_token token = consume(rest);
        return bee_ast_node_new_unary(token, BEE_AST_NODE_UNA_ARI_NEG,
                                      bee_parse_unary(rest, error));
    }

    if (match_punct(*rest, BEE_PUNCT_TILDE)) {
        struct bee_token token = consume(rest);
        return bee_ast_node_new_unary(token, BEE_AST_NODE_UNA_BIT_NEG,
                                      bee_parse_unary(rest, error));
    }
...
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_call_or_lit(struct bee_token *rest,
                                           struct bee_error *error) {
    if (match_type(*rest, BEE_TOKEN_TYPE_NUMBER)) {
        ...
        const char *digits =
                num_data.data + num_data.off +
                (num_data.num_base == BEE_NUM_BASE_DEC ? (neg < 0 ? 1 : 0) : 2);
        ...
        switch (num_data.num_type) {
            case BEE_NUM_TYPE_U8:
                ...
                node->as_u8 =
                        (uint8_t) (strtoul(digits, NULL, base_value[num_data.num_base]) * neg);
                break;
            case BEE_NUM_TYPE_U16:
                node->type = BEE_AST_NODE_LIT_U16;
                node->as_u16 = (uint16_t) (
                        strtoul(digits, NULL, base_value[num_data.num_base]) * neg);
                break;
            ...
            case BEE_NUM_TYPE_I8:
                ...
                node->as_i8 =
                        (int8_t) (strtoul(digits, NULL, base_value[num_data.num_base]) * neg);
                break;
            case BEE_NUM_TYPE_I16:
                node->type = BEE_AST_NODE_LIT_I16;
                node->as_i16 =
                        (int16_t) (strtoul(digits, NULL, base_value[num_data.num_base]) * neg);
                break;
            case BEE_NUM_TYPE_I32:
                node->type = BEE_AST_NODE_LIT_I32;
                node->as_i32 =
                        (int32_t) (strtoul(digits, NULL, base_value[num_data.num_base]) * neg);
                break;
            case BEE_NUM_TYPE_I64:
                node->type = BEE_AST_NODE_LIT_I64;
                node->as_i64 =
                        (int64_t) (strtoul(digits, NULL, base_value[num_data.num_base]) * neg);
                break;
...
// call = primary ('(' args ')')*
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_call(struct bee_token *rest,
                                    struct bee_error *error) {
    ...
    for (;;) {
        if (match_punct(*rest, BEE_PUNCT_LPAR)) {
            ...
            if (!match_punct_and_consume(rest, BEE_PUNCT_RPAR)) {
                bee_set_error(error, BEE_ERROR_PARSER_WAS_EXPECTING_RPAR, rest->name,
                              rest->row, rest->col,
                              "Unexpected token `%.*s`, was expecting: `)`", rest->len,
                              rest->data + rest->off);
                return NULL;
...
// args = (expr ',')*
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_args(struct bee_token *rest,
                                    struct bee_error *error) {
    ...
    for (;;) {
        ...
        if (root != NULL && !match_punct_and_consume(rest, BEE_PUNCT_COMMA)) {
            bee_set_error(error, BEE_ERROR_PARSER_WAS_EXPECTING_COMMA, rest->name,
                          rest->row, rest->col,
                          "Unexpected token `%.*s`, was expecting: `,`", rest->len,
                          rest->data + rest->off);
            return NULL;
        ...
        if (node == NULL) {
            node = bee_ast_node_new_binary(*rest, BEE_AST_NODE_ARG,
                                           bee_parse_expr(rest, error), NULL);
            root = node;
        } else {
            node->right = bee_ast_node_new_binary(*rest, BEE_AST_NODE_ARG,
                                                  bee_parse_expr(rest, error), NULL);
            node = node->right;
        }
    }
...
// primary = '(' expr ')' | id
// NOLINTNEXTLINE(misc-no-recursion)
struct bee_ast_node *bee_parse_primary(struct bee_token *rest,
                                       struct bee_error *error) {
    ...
    bee_set_error(error, BEE_ERROR_PARSER_WAS_EXPECTING_EXPR, rest->name,
                  rest->row, rest->col,
                  "Unexpected token `%.*s`, was expecting an expression",
                  rest->len, rest->data + rest->off);
    return NULL;
}

struct bee_ast_node *bee_must_parse_id(struct bee_token *rest,
                                       struct bee_error *error) {
    if (!match_type(*rest, BEE_TOKEN_TYPE_ID)) {
        bee_set_error(error, BEE_ERROR_PARSER_WAS_EXPECTING_ID, rest->name,
                      rest->row, rest->col,
                      "Unexpected token `%.*s`, was expecting: <id>", rest->len,
                      rest->data + rest->off);
        return NULL;
    }
